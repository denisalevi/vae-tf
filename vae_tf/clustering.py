...
def assign_cluster_labels(cluster_indices, true_labels):
    ...
    occurences of the most often occuring label to all labels in that cluster.
    If a label is already assigned to another cluster, find the fraction of the
    next most often occuring label, postpone the label assignment until all
    clusters with higher fractions of most often occuring labels are assigned
    ...
    -------
    ndarray
        Array with shape of `cluster_indices` with the assigned labels.
    ...
    for n, idx in enumerate(sorted_unique_cluster_indices):
        ...
        for m, label in enumerate(label_options[offset:]):
            ...
            if label not in used_labels.keys():
                # TODO assert all(cluster_labels[cluster_masks[idx]] == -1), "Trying to overassign labels"
